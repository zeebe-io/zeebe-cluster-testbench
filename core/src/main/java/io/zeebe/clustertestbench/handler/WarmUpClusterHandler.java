...
public class WarmUpClusterHandler implements JobHandler {
	...
	public void handle(JobClient client, ActivatedJob job) throws Exception {
		...
		final CamundaCloudAuthenticationDetails authenticationDetails = input.getAuthenticationDetails();
		final OAuthCredentialsProvider cred = buildCredentialsProvider(requireNonNull(authenticationDetails));

		try (final ZeebeClient zeebeClient = ZeebeClient.newClientBuilder()
				.brokerContactPoint(authenticationDetails.getContactPoint()).credentialsProvider(cred).build()) {
			...
			zeebeClient.newWorker().jobType(JOB_TYPE).handler(new MoveAlongJobHandler()).timeout(Duration.ofMinutes(1))
					.open();
